...
export class ProductService {
  ...
  async create(body: CreateProductDto, file:Express.Multer.File) {
    ...
    const newData = {advert_sell:sell, advert_buy:buy, avert_picture:file.filename, advert_url: url}
  ...
  async update(id: string, body: UpdateProdDto) {
    const {buy,sell,url} = body;
    const data = await this.knex("advert").update({
     advert_buy :buy,
     advert_sell: sell,
     advert_url: url 

    }).where({advert_id:id}).returning("*")
    return {message: data};
  }
